...
// arr.sort() 배열 재정렬 -> 배열 자체가 변경된다. 인수로 정렬 로직을 담은 함수를 받는다.
let arr18 = [1, 5, 3, 4, 2];
console.log(arr18.sort());  // [1, 2, 3, 4, 5]
let arr19 = [23, 7, 8, 13, 50];
console.log(arr19.sort());  // [13, 23, 50, 7, 8] => 정렬 할 때 요소를 문자열로 취급하기에 첫번째자리를 기준으로 정렬한다.
...
console.log(arr19.sort(fn));  // [7, 8, 13, 23, 50]
// Lodasg 정렬 라이브러리
// _.sortBy(arr);


// arr.reduce() 배열의 모든 수 합치기 인수로 함수를 받는다. (누적된 계산값, 현재값) => { return 계산값 }
let arr20 = [1, 2, 3, 4, 5];
const result5 = arr20.reduce((prevent, current) => {
    return prevent + current;
}, 0); // <= 초기값 설정, 초기값을 설정하지 않을 경우 인수의 첫번째 값이 초기값이 된다.
...
const result6 = userList.reduce((prevent, current) =>{
    if(current.age > 19){
        prevent.push(current.name);
    }
    return prevent;
}, []);
console.log(result6);   // ['Bro', 'Kim', 'Nil']